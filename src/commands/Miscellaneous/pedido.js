...
module.exports = class prefix extends Command {
  ...
  async run(message, args, prefix, author, channel) {
    const subs = args[0] && this.client.commands.subcommands
    .get(this.reference)
    .find((cmd) => cmd.name.toLowerCase() === args[0].toLowerCase() ||
    ...
    if(!subs) {
      sub = "null"
      subcmd = sub && this.client.commands.subcommands
      .get(this.reference)
      .find((cmd) => cmd.name.toLowerCase() === sub.toLowerCase() ||
